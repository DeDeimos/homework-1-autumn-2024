...
- [**Регистрация (ссылка)**](https://jimder.ru/register):
  ...
  - Поле **Дата рождения**:
    ...
    - При вводе данных происходит валидация: дата в формате системы с 1930г по 2008г. ✅
  - Чекбоксы **Ваш пол**:
    - Если пол не выбран, то отображается ошибка ("Выберите пол") ✅
  ...
  - Чекбоксы **интересов**:
    - Если ни один интерес не выбран, то отображается ошибка ("Выберите хотя бы один интерес") ✅
...
- **Вкладка "Профиль"**:
  ...
  - **Интересы**:
    ...
    - Если произошла ошибка на стороне сервера, отображается сообщение "Что-то пошло не так". ✅
  ...
  - **Биография**:
    ...
    - Если ошибки есть, то выводится соответствующее сообщение об ошибке (Если поле пустое: "Поле не может быть пустым", если ошибка на сервере: "Что-то пошло не так"). Обновления не происходит. ✅
...
- **Вкладка "Безопасность"**:
  - **Редактирование имени**:
    ...
    - При заполнении поля происходит проверка корректности (Длина 2-32 символа любого алфавита, 1 пробел или дефис между словами, никаких спецсимволов), при ошибке отображается соответствующее сообщение ("Поле не может быть пустым" или "Поле некорректно") + подсказка. ✅
...
- **Кнопка "Выход"**:
  - При нажатии на кнопку "Выход" открывается модальное окно с подтверждением, при нажатии кнопки "Продолжить" происходит выход из системы. ✅
  - Если действие авторизационной cookie закончилось, то при данном условии при первой загрузке страницы, если выход предварительно не совершался, автоматический выход из системы не происходит (перенаправление на страницу авторизации), а при нажатии "Продолжить" появляется ошибка "Что-то пошло не так".⚠️
    > **[Баг]**
    >
...
Когда гость не авторизован, то он не может зайти на страницы авторизованного. Его сразу перенаправит на страницу логина. Точно также работает и в обратную сторону для авторизованных, пользователя будет перекидывать на страницу с карточками.
